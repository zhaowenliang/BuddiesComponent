 ...
 * 网络状态变化监听管理器
 * <pre>
 *     1. 使用前需要先注册该管理器，推荐应用启动时在Application中注册：
 *         NetworkStateManager.getInstance().register(applicationContext);
 *     2. 在需要监听网络状态变化的页面使用getNetStateLiveData()获取LiveData进行网络状态变化监听：
 *         NetworkStateManager.getInstance().getNetStateLiveData().observe(this, netState -> {...});
 * </pre>
 */
public class NetworkStateManager {

    private static final String TAG = "网络状态监听器";
